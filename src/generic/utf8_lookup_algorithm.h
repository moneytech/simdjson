...
really_inline simd8<uint8_t> check_utf8(const simd8<uint8_t> input, const simd8<uint8_t> prev_input) {
...
// Tell if this input is incomplete (if it needs continuation bytes in the next block)
...
using namespace utf8_validation;
...
private:
  ...
  really_inline void check_utf8(const simd8x64<uint8_t> input, const simd8<uint8_t> prev_input) {
    this->error |= utf8_validation::check_utf8(input.chunks[0], prev_input);
    for (int i=1; i<simd8x64<uint8_t>::NUM_CHUNKS; i++) {
      this->error |= utf8_validation::check_utf8(input.chunks[i], input.chunks[i-1]);
    }
...
public:
  ...
  really_inline void check_first(const simd8x64<uint8_t> input) {
    if (unlikely(has_utf8(input))) {
      this->check_utf8(input, uint8_t(0));
      return;
    }
    // We don't bother checking ASCII on the first block. It's definitely OK.
  }

  really_inline void check_next(const simd8x64<uint8_t> input, const simd8<uint8_t> prev_bytes, UNUSED const uint8_t *buf) {
    if (unlikely(has_utf8(input))) {
      this->check_utf8(input, prev_bytes);
      return;
    }
