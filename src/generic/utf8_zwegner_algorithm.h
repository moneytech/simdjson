...
struct utf8_checker {
  ...
  //
  really_inline void check_special_cases(const simd8<uint8_t> bytes, const simd8<uint8_t> shifted_bytes) {
  ...
  really_inline void check_utf8_bytes(const simd8<uint8_t> bytes, const simd8<uint8_t> shifted_bytes, const vmask_t bit_7) {
    this->check_length_errors(bytes, bit_7);
    this->check_special_cases(bytes, shifted_bytes);
  }

  really_inline void check_chunk(const simd8x64<uint8_t> in, const uint8_t* buf, int i) {
    vmask_t bit_7 = in.chunks[i].get_bit<7>();
    if (unlikely(bit_7)) {
      this->check_utf8_bytes(in.chunks[i], &buf[sizeof(simd8<uint8_t>)*i-1], bit_7);
    } else {
      this->length_errors |= this->last_cont;
    }
  }

  really_inline void check_first(const simd8x64<uint8_t> in, const uint8_t* buf) {
    vmask_t bit_7 = in.chunks[0].get_bit<7>();
    if (unlikely(bit_7)) {
      this->check_utf8_bytes(in.chunks[0], in.chunks[0].prev(), bit_7);
    }

    for (int i=1;i<simd8x64<uint8_t>::NUM_CHUNKS;i++) {
      this->check_chunk(in, buf, i);
    }
  }

  really_inline void check_next(const simd8x64<uint8_t> in, UNUSED simd8<uint8_t> prev_bytes, const uint8_t* buf) {
    for (int i=0;i<simd8x64<uint8_t>::NUM_CHUNKS;i++) {
      this->check_chunk(in, buf, i);
    }
  }

  // The last chunk is always a remainder chunk, so you can't look back before it.
  really_inline void check_last(const simd8x64<uint8_t> in, simd8<uint8_t> prev_bytes, const uint8_t* buf) {
    vmask_t bit_7 = in.chunks[0].get_bit<7>();
    if (unlikely(bit_7)) {
      this->check_utf8_bytes(in.chunks[0], in.chunks[0].prev(prev_bytes), bit_7);
    } else {
      this->length_errors |= this->last_cont;
    }
