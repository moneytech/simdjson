...
public:
  ...
  really_inline simd8<uint8_t> scan_step(const uint8_t *buf, const size_t idx, utf8_checker &utf8_checker, simd8<uint8_t>* prev_bytes, bool is_last);

  template<size_t STEP_SIZE>
  really_inline simd8<uint8_t> scan_first(const uint8_t *buf, utf8_checker &utf8_checker) {
    return this->scan_step<STEP_SIZE>(buf, 0,   utf8_checker, NULL,        false);
  }

  template<size_t STEP_SIZE>
  really_inline simd8<uint8_t> scan_next(const uint8_t *buf, const size_t idx, utf8_checker &utf8_checker, simd8<uint8_t> prev_bytes) {
    return this->scan_step<STEP_SIZE>(&buf[idx], idx, utf8_checker, &prev_bytes, false);
  }

  template<size_t STEP_SIZE>
  really_inline ErrorValues scan_remainder(const uint8_t *buf, const size_t idx, const size_t len, utf8_checker &utf8_checker, simd8<uint8_t>* prev_bytes) {
    uint8_t end_buf[STEP_SIZE];
    memset(end_buf, 0x20, STEP_SIZE);
    memcpy(end_buf, &buf[idx], len - idx);
    this->scan_step<STEP_SIZE>(end_buf, idx, utf8_checker, prev_bytes, true);
    return this->scan_eof(idx+STEP_SIZE, len, utf8_checker);
  }

  template<size_t STEP_SIZE>
  really_inline ErrorValues scan_last(const uint8_t *buf, const size_t idx, const size_t len, utf8_checker &utf8_checker, simd8<uint8_t> prev_bytes) {
    return this->scan_remainder<STEP_SIZE>(buf, idx, len, utf8_checker, &prev_bytes);
  }

  template<size_t STEP_SIZE>
  really_inline ErrorValues scan_single(const uint8_t *buf, const size_t len, utf8_checker &utf8_checker) {
    return this->scan_remainder<STEP_SIZE>(buf, 0, len, utf8_checker, NULL);
  }
...
//
template<>
really_inline simd8<uint8_t> json_structural_scanner::scan_step<128>(const uint8_t *buf, const size_t idx, utf8_checker &utf8_checker, simd8<uint8_t>* prev_bytes, bool is_last) {
  //
  ...
  utf8_checker.check_next(in_2, in_1.last_chunk(), buf+64);
...
//
template<>
really_inline simd8<uint8_t> json_structural_scanner::scan_step<64>(const uint8_t *buf, const size_t idx, utf8_checker &utf8_checker, simd8<uint8_t>* prev_bytes, bool is_last) {
  //
  ...
  if (idx == 0) {
    ...
    if (is_last) {
      ...
      // Only pass the buffer is there is something behind it (it's there to get the previous bytes!)
      utf8_checker.check_next(in_1, *prev_bytes, &buf[idx]);
    }
    this->structural_indexes.write_indexes(idx-64, this->prev_structurals); // Output *last* iteration's structurals to ParsedJson
  }
  this->prev_structurals = structurals_1 & ~string_1;
...
really_inline ErrorValues json_structural_scanner::scan(const uint8_t *buf, const size_t len, utf8_checker &utf8_checker) {
  ...
  if (likely(len > STEP_SIZE)) {
    ...
    simd8<uint8_t> prev_bytes = this->scan_first<STEP_SIZE>(buf, utf8_checker);

    // Loop until all except the last chunk.
    size_t idx;
    for (idx = STEP_SIZE; idx < (len - STEP_SIZE); idx += STEP_SIZE) {
      prev_bytes = this->scan_next<STEP_SIZE>(buf, idx, utf8_checker, prev_bytes);
    }

    // The last step will be between 1 and STEP_SIZE bytes.
    return this->scan_last<STEP_SIZE>(buf, idx, len, utf8_checker, prev_bytes);
